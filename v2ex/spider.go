		...
		for i := nextNumber; i < siteState.MemberMax; i++ {
			...
			if re != nil {
				...
				if re.StatusCode() == http.StatusNotFound {
					core.Logger.Infof("[spider] member[%d] not found, insert fake member instead.\n", i)
					...
					// 继续处理下一条数据
					core.Logger.Infof("[spider] insert fake member[%d] success\n", i)
					continue
				}
				...
				if re.RateLimit() {
					...
					core.Logger.Infof("[spider] member[%d] rate limit occured, retry after 10 mintus.\n", i)
			...
			if e != nil {
				core.Logger.Infof("[spider] insert real member[%d] error, retry now. error: %v\n", i, e)
				i--
				continue
			}
			core.Logger.Infof("[spider] insert real member[%d-%s] success, go next.\n", i, member.Name)
		}
	}
}
